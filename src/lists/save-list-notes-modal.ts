/**
 * Modal de confirmación de «Guardar esta nota como notas de la lista».
 *
 * Gemelo de `notes/save-note-modal.ts`: misma composición (`note-snapshot.ts`),
 * mismo trato del tope de Lumbre (preguntar antes de recortar, nunca en
 * silencio), cambiando «tarea» por «lista». El modal no habla con la red ni
 * con la cola: solo compone el texto final y se lo pasa al llamador.
 */
...
import {
	...
	snapshotHeader,
	type NoteSnapshot,
} from '../notes/note-snapshot';

export interface SaveListNotesModalOptions {
	...
	text: string;
	/** Nombre de la lista, para «a qué lista va». */
	listName: string;
	/** Las notas ACTUALES de la lista, para contar fotos anteriores y unir. */
...
export class SaveListNotesModal extends Modal {
	...
	constructor(
		app: App,
		private readonly options: SaveListNotesModalOptions,
	) {
		super(app);
	...
	onOpen(): void {
		...
		this.setTitle('Guardar esta nota como notas de la lista');
		this.render();
	}
	...
	private render(): void {
		...
		info.createDiv({ cls: 'lumbre-save-note-modal__task', text: `Lista: ${this.options.listName}` });
		info.createDiv({
			text:
		...
		if (previous > 0) {
			info.createDiv({
				...
				text: `Ya hay ${previous} ${previous === 1 ? 'foto anterior' : 'fotos anteriores'} en esta lista.`,
			});
		}
